...
public class QmSecurityTokenTools {
    ...
    public static String createToken(QmUserInfo qmUserInfo) throws QmSecurityCreateTokenException {
        try {
            ...
            token.append(QmAesEncDecTools.encrypt(identity, SecurityConstants.BASIC_KEY));
            token.append("%@header@%");
            token.append(QmAesEncDecTools.encrypt(qmUserInfo.getUser(), SecurityConstants.BASIC_KEY));
            token.append("%@body@%");
            token.append(QmAesEncDecTools.encrypt(String.valueOf(qmUserInfo.getTokenExpireTime()), SecurityConstants.BASIC_KEY));
            token.append("%@tokenExp@%");
            token.append(QmAesEncDecTools.encrypt(signTime, SecurityConstants.BASIC_KEY));
    ...
    public static QmUserInfo analysisToken(String token) throws QmSecurityAnalysisTokenException {
        try {
            ...
            String identify = QmAesEncDecTools.decrypt(temp[0], SecurityConstants.BASIC_KEY);
            tokenTemp = temp[1];
            temp = tokenTemp.split("%@body@%");
            String userJson = QmAesEncDecTools.decrypt(temp[0], SecurityConstants.BASIC_KEY);
            tokenTemp = temp[1];
            temp = tokenTemp.split("%@tokenExp@%");
            String tokenExp = QmAesEncDecTools.decrypt(temp[0], SecurityConstants.BASIC_KEY);
            tokenTemp = temp[1];
            temp = tokenTemp.split("%@signTime@%");
            String signTime = QmAesEncDecTools.decrypt(temp[0], SecurityConstants.BASIC_KEY);
            String sign = temp[1];
            // 封装对象
    ...
    private static void sendResponseToken(HttpServletResponse response, String token) {
        ...
        LOG.debug("设置了Response " + QmSecurityContent.getHeaderTokenKeyName() + "响应头!");
    }
